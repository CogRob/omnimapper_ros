...
namespace scan_tools {
     ...
     *
     */
    bool CanonicalScan::processScan(LDP& curr_ldp_scan, LDP& prev_ldp_scan,
	...
	bool valid = true;
	icp_cov =
	  gtsam::noiseModel::Gaussian::Covariance(upgrade_pose2_pose3_cov(gsl_to_gtsam_matrix(output_.cov_x_m, valid)));
					...
					LDP& ldp)
{
  ...
  void CanonicalScan::PointCloudToLDP(const sensor_msgs::PointCloud& cloud,
    ...
    unsigned int n = scan.ranges.size();//cloud.points.size();//width * cloud.height ;
    ldp = ld_alloc_new(n);
    if (n > 0)  {
