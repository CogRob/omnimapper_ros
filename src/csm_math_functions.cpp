...
gtsam::Pose3 GetPose(const tf::StampedTransform& transform) {
  ...
  if (fabs(axis_norm - 1.0) < 0.01) {
    ...
    gtsam::Vector axis_vector(3);
    axis_vector << axis[0]/axis_norm, axis[1]/axis_norm, axis[2]/axis_norm;
    gtsam_axis = (axis_vector);
...
GetOdoCovariance( const gtsam::Pose3& odo,
  ...
  noise_vector <<  a_scale*(sroll), a_scale*(spitch), a_scale*(syaw),
					         l_scale*(sx), l_scale*(sy), l_scale*(sz);
